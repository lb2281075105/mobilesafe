...
//        }
    ...
    private void initUI() {
        ...
        // 扫描显示的文字
        tv_name = findViewById(R.id.tv_saomiao);
        // listview
        ll_cache_clear = findViewById(R.id.ll_cache_clear);
//        缓存清理
        btn_qingli = findViewById(R.id.btn_qingli);
        // sd卡清理
        btn_sd = findViewById(R.id.btn_sd);

        btn_liji.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                //1.获取指定类的字节码文件
                try {
                    Class<?> clazz = Class.forName("android.content.pm.PackageManager");
                    //2.获取调用方法对象
//                    Method methodod = clazz.getMethod("freeStorageAndNotify", long.class,IPackageDataObserver.class);
                    //3.获取对象调用方法
